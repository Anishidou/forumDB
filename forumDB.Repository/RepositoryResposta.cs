...
namespace forumDB.Repository
{
    public class RepositoryResposta
    {
        ...
        public void ExcluirRespostasPorUsuario(int id)
        {
            foreach (Resposta oResposta in odb.Resposta.Where(x => x.IdUsuario == id).ToList())
            {
                oResposta.IdUsuario = 1;
                odb.Entry(oResposta).State = EntityState.Modified;
                odb.SaveChanges();
            }
        }

        public void ExcluirRespostasPorPergunta(int id)
        {
            foreach (Resposta oResposta in odb.Resposta.Where(x => x.IdPergunta == id).ToList())
            {
                odb.Entry(oResposta).State = EntityState.Deleted; 
                odb.SaveChanges();
            }
